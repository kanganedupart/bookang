...
try {
  for (const viewport of [{ name: "pc", width: 1365, height: 900 }, { name: "mobile", width: 390, height: 844 }]) {
    ...
    const ecodingDateRules = await page.evaluate(() => {
      ...
      const alignmentState = structuredClone(st);
      alignmentState.students = { MANUAL:{id:"MANUAL",name:"(반5)우태양7206",active:true,createdPeriodId:pid,onboarding:true,holdings:{KEEP:1},periodClasses:{[pid]:{COLD:"COLD"}},periodMembership:{[pid]:true},classes:{COLD:"COLD"}} };
      alignmentState.books = { KEEP:{id:"KEEP",stock:7} }; alignmentState.movements = { M1:{id:"M1"} };
      alignmentState.ecodingClassMappings = {[pid]:{EXTNEW:{localClassId:"CNEW"},EXTOLD:{localClassId:"COLD"}}};
      alignmentState.ecodingEvents = { E1:{id:"E1",periodId:pid,type:"NEW",status:"PENDING",studentName:"(반5)우태양7206",externalStudentId:"EXT-STUDENT"} };
      const alignment = alignInitialEcodingRoster(alignmentState,pid,new Map([["EXT-STUDENT",{externalId:"EXT-STUDENT",name:"(반5)우태양7206",number:"7206",classIds:{CNEW:"CNEW"}}]]),{membershipSchemaVersion:"CLASS_DATES_V1",syncedAt:"2026-09-07T12:00:00+09:00"});
